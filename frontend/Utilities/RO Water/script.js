document.addEventListener("DOMContentLoaded", () => {
    ...
    async function updatePressureStatus() {
        try {
            ...
            if (data.length > 0) {
                document.getElementById("kpi-pressure").textContent = data[data.length - 1].bar.toFixed(1);
    ...
    async function fetchPressureByDate(start, end, isAutomatic = false) {
        try {
            ...
            if (data.length === 0) {
                document.getElementById("kpi-pressure").textContent = "--";
                renderChart('pressureChart', 'line', { labels: [], datasets: [{ label: 'RO Supply', data: [], borderColor: '#3b82f6', tension: 0.3 }] });
                return;
            }
            document.getElementById("kpi-pressure").textContent = data[data.length - 1].bar.toFixed(1);
            renderChart('pressureChart', 'line', {
                labels: data.map(d => d.time),
                datasets: [{ label: 'RO Supply', data: data.map(d => d.bar), borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', fill: false, tension: 0.3, borderWidth: 2 }]
            });
    ...
    async function renderZoomChart(type, start, end) {
        ...
        const label    = type === 'pressure' ? 'RO Supply (bar)' : 'Residual Cl2 (mg)';
        ...
        zoomedChart = new Chart(ctx, {
            ...
            data: {
                labels: data.map(d => d.time),
                datasets: [{ label, data: data.map(d => d[valueKey]), borderColor: color, backgroundColor: color.replace(')', ',0.1)').replace('rgb', 'rgba'), fill: type === 'chlorine', tension: 0.3, borderWidth: 2 }]
            },
            options: {
