document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchPressureByDate(start, end, isAutomatic = false) {
        try {
            ...
            if (data.length === 0) {
                document.getElementById("kpi-ro-pres").textContent = "--";
                renderChart('pressureChart', 'line', { labels: [], datasets: [{ label: 'Softwater 2', data: [], borderColor: '#f59e0b', tension: 0.3 }] });
                return;
            }
            document.getElementById("kpi-ro-pres").textContent = data[data.length - 1].bar.toFixed(1);
            renderChart('pressureChart', 'line', {
                labels: data.map(d => d.time),
                datasets: [{ label: 'Softwater 2 (bar)', data: data.map(d => d.bar), borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,0.1)', fill: false, tension: 0.3, borderWidth: 2 }]
            });
    ...
    async function openZoom(type) {
        ...
        zoomTitleEl.textContent = type === 'pressure' ? 'Softwater 2 Pressure (bar)' : 'Chlorine Monitoring (mg)';
    ...
    async function renderZoomChart(type, start, end) {
        ...
        const color    = '#f59e0b';
        const label    = type === 'pressure' ? 'Softwater 2 (bar)' : 'Residual Cl2 (mg)';
        ...
        zoomedChart = new Chart(ctx, {
            ...
            data: {
                labels: data.map(d => d.time),
                datasets: [{ label, data: data.map(d => d[valueKey]), borderColor: color, backgroundColor: 'rgba(245,158,11,0.1)', fill: type === 'chlorine', tension: 0.3, borderWidth: 2 }]
            },
            options: {
    ...
    function updateKPIs(data) {
        document.getElementById("kpi-ro-total").textContent = (data.flow_totals.soft_water_2?.slice(-1)[0]?.m3 || 0).toLocaleString();
    }
